...
Determine if there is a path from (1, 1) to (n, m) so that the sum of the path is 0.
...
void solve(){
  ...
  mn_dp(n + 1, vector<int>(m + 1, 2*(n + m))), mx_dp(n + 1, vector<int>(m + 1, -2*(n + m)));
  ...
  if (mn_dp[n][m] <= 0 && mx_dp[n][m] >= 0) cout << "YES";
  else cout << "NO";
}
